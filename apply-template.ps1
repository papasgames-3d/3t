# Script to apply the modern template to all game HTML files
# This script will:
# 1. Create backups of all original game HTML files
# 2. Extract the unique content from each game (title, iframe source, etc.)
# 3. Apply the new template while preserving that content

# Display script start message
Write-Host "Starting to apply modern template to all game HTML files..." -ForegroundColor Yellow
...
New-Item -ItemType Directory -Path "$backupDir\game" -Force | Out-Null
...
foreach ($file in $gameFiles) {
    ...
    $relativeFilePath = $file.FullName.Replace((Get-Location).Path + "\", "")
    $backupFilePath = Join-Path -Path (Get-Location).Path -ChildPath $backupDir -AdditionalChildPath $relativeFilePath
    ...
    try {
        ...
        $backupFolder = Split-Path -Path $backupFilePath -Parent
        if (-not (Test-Path $backupFolder)) {
            New-Item -ItemType Directory -Path $backupFolder -Force | Out-Null
        }
        
        # Create a backup copy
        Copy-Item -Path $originalFilePath -Destination $backupFilePath -Force
        # Also create a .backup version in the original location
        Copy-Item -Path $originalFilePath -Destination "$originalFilePath.backup" -Force
        
        # Read the content of the original file
        $content = Get-Content -Path $originalFilePath -Raw
        
        # Extract the important information from the original file
        $titleMatch = [regex]::Match($content, '<title>(.*?)</title>')
        $title = if ($titleMatch.Success) { $titleMatch.Groups[1].Value } else { "Game Title" }
        
        $h1Match = [regex]::Match($content, '<h1[^>]*?>(.*?)</h1>')
        $h1Title = if ($h1Match.Success) { $h1Match.Groups[1].Value } else { $title }
        
        $iframeMatch = [regex]::Match($content, '<iframe[^>]*?src="([^"]*)"[^>]*?>')
        $iframeSrc = if ($iframeMatch.Success) { $iframeMatch.Groups[1].Value } else { "" }
        
        $descriptionMatch = [regex]::Match($content, '<meta name="description"[^>]*?content="([^"]*)"[^>]*?>')
        $description = if ($descriptionMatch.Success) { $descriptionMatch.Groups[1].Value } else { "Play this game instantly in your browser for free!" }
        
        # Create relative paths for CSS and assets (going up one level from game directory)
        $relativePath = "../"
...
"@
        ...
        Set-Content -Path $originalFilePath -Value $newContent
        
        Write-Host "[$processed/$($gameFiles.Count)] Successfully processed: $relativeFilePath" -ForegroundColor Green
        $successful++
    }
    catch {
        Write-Host "[$processed/$($gameFiles.Count)] ERROR processing: $relativeFilePath" -ForegroundColor Red
...
if ($failed -gt 0) {
    Write-Host "`nSome files failed to update. You can restore from the backup directory: $backupDir" -ForegroundColor Yellow
}
else {
    Write-Host "`nBackup created at: $backupDir" -ForegroundColor Cyan
}

Write-Host "`nTo restore files from backup, use restore-backup.ps1" -ForegroundColor Yellow 