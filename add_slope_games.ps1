# Script de them game tu slopegame-3d.com

# Danh sach game theo danh muc
$gameCategories = @{
    "car" = @(
        @{
            name = "Drift Hunters"
...
<head>
    ...
    <meta name="description" content="{0} - Play AT Monkey Mart One: Enjoy browser play, fullscreen action, and an ad-free gaming experience. Dive into fun today!">
    ...
    <div class="container">
        <div class="gameFrame">
            ...
            <div class="game-container">
                <iframe class="game-iframe" id="gameFrame" src="{1}" style="background:black;" width="100%" height="600" scrolling="no" frameborder="0"></iframe>
            </div>
...
function Create-GameFile {
    ...
    $gameContent = $gameTemplate -f $name, $iframe
    $gameContent | Out-File -FilePath $filePath -Encoding UTF8
    
    Write-Host "Da tao file game: $filePath" -ForegroundColor Green
    
    # Them game vao trang danh muc
    $categoryFile = "category/$category.html"
    if (Test-Path $categoryFile) {
        $content = Get-Content $categoryFile -Raw
        
        # Tao HTML cho game item
        $gameItem = @"
        <div class="col-sm-6 col-md-4 col-lg-2 game-item">
            <a class="game-link" href="/game/$safeName.html">
                <img class="img-fluid game-card__cover" src="../assets/upload/webglmath/jpg/$($safeName).jpg">
                <h3 class="game-card__title" style="font-size: 15px;">$name</h3>
            </a>
        </div>
"@
        
        # Them vao truoc </div> cuoi cung
        $content = $content -replace '</div>\s*</div>\s*</div>\s*$', "$gameItem`n</div></div></div>"
        $content | Set-Content $categoryFile -Force
        
        Write-Host "Da them game vao danh muc $category" -ForegroundColor Green
    }
}

# Tao game cho tung danh muc
foreach ($category in $gameCategories.Keys) {
    Write-Host "`nDang them game vao danh muc $category..." -ForegroundColor Yellow
    
    foreach ($game in $gameCategories[$category]) {
        Create-GameFile -name $game.name -iframe $game.iframe -category $category
    }
}

Write-Host "`nHoan tat them game!" -ForegroundColor Green 